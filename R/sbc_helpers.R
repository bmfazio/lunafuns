...
#   }
#
#   stanmodel_brms_savedir <- (\(){
#     sm_tmp <- SBC:::stanmodel_for_brms
#     body(sm_tmp)[[8]][[3]][[2]] <- substitute(compiled_model <- cmdstanr::cmdstan_model(cmdstanr::write_stan_file(model_code, dir = "models/autogenerated")))
#     environment(sm_tmp) <- parent.frame()
#     sm_tmp
#   })()
#
#   backend_obj <- do.call((\(){
#     bk_tmp <- SBC::SBC_backend_brms
#     body(bk_tmp)[[4]] <- substitute(SBC:::validate_SBC_backend_brms_args(args))
#     # test if "unfreezing" the compiled file fixes my shit
#     #body(bk_tmp)[[5]] <- substitute(stanmodel <- stanmodel_brms_savedir(data = template_data, ...))
#     body(bk_tmp)[[6]] <- substitute(stanmodel <- SBC:::new_SBC_backend_brms(stanmodel, args))
#     # environment(bk_tmp) <- parent.frame()
#     bk_tmp
#   })(),
#   c(dots, list(template_data = template_data)))
#
